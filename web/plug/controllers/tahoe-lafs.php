...
function install()
{
    ...
    if (isPackageInstall($TAHOE_VARS['PACKAGE_NAME'])) {
        ...
        foreach (execute_program('addgroup --system tahoe')['output'] as $key => $value) {
            $postInstall[] = $value;
        }
        foreach (execute_program('adduser --system --ingroup tahoe --home '.$TAHOE_VARS['DAEMON_HOMEDIR'].' --shell '.$TAHOE_VARS['DAEMON_SHELL'].' '.$TAHOE_VARS['DAEMON_USERNAME'])['output'] as $key => $value) {
            $postInstall[] = $value;
        }
        execute_program_shell('touch ' .$TAHOE_VARS['DAEMON_HOMEDIR']);
        foreach (execute_program('chown -vR '.$TAHOE_VARS['DAEMON_USERNAME'].':'.$TAHOE_VARS['DAEMON_GROUP'].' '.$TAHOE_VARS['DAEMON_HOMEDIR'])['output'] as $key => $value) {
            $postInstall[] = $value;
        }
        ...
        foreach (execute_program('chmod -v +x '.$TAHOE_VARS['TAHOE_ETC_INITD_FILE'])['output'] as $key => $value) {
            $postInstall[] = $value;
        }
...
function uninstall()
{
    ...
    foreach (execute_program('update-rc.d tahoe-lafs remove')['output'] as $key => $value) {
        $postUninstall[] = $value.'<br/>';
    }
    ...
    foreach (execute_program('deluser --system --remove-home tahoe')['output'] as $key => $value) {
        $postUninstall[] = $value.'<br/>';
    }
    foreach (execute_program('delgroup --system tahoe')['output'] as $key => $value) {
        $postUninstall[] = $value.'<br/>';
    }
    $postUninstall[] = execute_program_shell('rm -rvf '.$TAHOE_VARS['DAEMON_HOMEDIR'])['output'] ;

    foreach ($postUninstall as $v) {
        $postUninstallAll .= $v;
    }
...
function createIntroducer_post()
{
    ...
    $postCreate = array();
    foreach (execute_program($TAHOE_VARS['TAHOE_COMMAND'].' create-introducer '.$TAHOE_VARS['DAEMON_HOMEDIR'].'/'.$TAHOE_VARS['INTRODUCER_DIRNAME'])['output'] as $k => $v) {
        $postCreate[] = $v;
    }
    ...
    foreach (execute_program('chown -vR tahoe:tahoe '.$TAHOE_VARS['DAEMON_HOMEDIR'].'/'.$TAHOE_VARS['INTRODUCER_DIRNAME'])['output'] as $key => $value) {
        $postCreate[] = $value;
    }

    $postCreateAll = "";
    foreach ($postCreate as $k => $v) {
        $postCreateAll .= $v.'<br/>';
    }
    ...
    $postStart = array();
    foreach (execute_program_shell($TAHOE_VARS['TAHOE_ETC_INITD_FILE'].' start '.$TAHOE_VARS['INTRODUCER_DIRNAME'])['output'] as $k => $v) {
        $postStart[] = $v;
    }

    //This pause is needed in order to let the server start before showing the success/error text
    sleep(2);
    foreach (execute_program_shell('ps aux | grep tahoe | grep introducer | grep python | grep -v grep') as $k => $v) {
        $postStart[] = $v;
    }
...
function createNode_get()
{
    ...
    if (!tahoeCreated($TAHOE_VARS['DAEMON_HOMEDIR'], $TAHOE_VARS['NODE_DIRNAME'])) {
        ...
        } elseif (tahoeCreated($TAHOE_VARS['DAEMON_HOMEDIR'], $TAHOE_VARS['INTRODUCER_DIRNAME'])) {
...
function createNode_post()
{
    ...
    $postCreate = array();
    foreach (execute_program($TAHOE_VARS['TAHOE_COMMAND'].' create-node '.$TAHOE_VARS['DAEMON_HOMEDIR'].'/'.$TAHOE_VARS['NODE_DIRNAME'])['output'] as $k => $v) {
        $postCreate[] = $v;
    }
    ...
    foreach (execute_program('chown -vR tahoe:tahoe '.$TAHOE_VARS['DAEMON_HOMEDIR'], $TAHOE_VARS['NODE_DIRNAME'])['output'] as $key => $value) {
        $postCreate[] = $value;
    }
    $postCreateAll = "";
    foreach ($postCreate as $k => $v) {
    ...
    $postStart = array();
    foreach (execute_program('/etc/init.d/tahoe-lafs start node')['output'] as $k => $v) {
        $postStart[] = $v;
    }
    //This pause is needed in order to let the server start before showing the success/error text
    sleep(2);
    foreach (execute_program('ps aux | grep tahoe | grep node | grep -v grep')['output'] as $k => $v) {
        $postStart[] = $v;
    }
...
function deleteIntroducer()
{
    ...
    foreach (execute_program('rm -vrf '.$TAHOE_VARS['DAEMON_HOMEDIR'].'/'.$TAHOE_VARS['INTRODUCER_DIRNAME'].'/*')['output'] as $k => $v) {
        $postDelete[] = $v;
    }
...
function deleteNode()
{
    ...
    $postDelete = array();
    $postDeleteAll = "";
    foreach (execute_program('rm -vrf '.$TAHOE_VARS['DAEMON_HOMEDIR'].'/'.$TAHOE_VARS['NODE_DIRNAME'].'/*')['output'] as $k => $v) {
        $postDelete[] = $v;
    }
